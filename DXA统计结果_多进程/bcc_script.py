...
def compute_DXA(single_frame:str):
  ...
  # 添加DXA分析模块, 注意这里我们需要手动选择的DXA输入晶体类型为BCC
  pipeline.modifiers.append(DXA(input_crystal_structure=DXA.Lattice.BCC))
  # 开始计算
  data = pipeline.compute(frame=0)
  # 获取当前轨迹的帧数
  step:int = data.attributes['Timestep']
  # 获取不同种类的位错的长度
  length_of_1_2_111:float = data.attributes['DislocationAnalysis.length.1/2<111>']
  length_of_100:float = data.attributes['DislocationAnalysis.length.<100>']
  length_of_110:float = data.attributes['DislocationAnalysis.length.<110>']
  ...
  # 将结果打包返回
  return (step, length_of_1_2_111, length_of_100, length_of_110, length_of_other, length_of_total)
...
if __name__ == '__main__':
  ...
  # 打开文件将结果写入到txt里面
  with open("DXA_result_BCC.txt", 'w') as fdata:
    fdata.write('#Step 1/2<111> <100> <110> other Total\n')
    for length in result:
      fdata.write(f'{length[0]} {length[1]} {length[2]} {length[3]} {length[4]} {length[5]}\n')
