...
def create_eam(atnames):
    ...
    #     parser.print_help()
    #     sys.exit("")
    nr = 2000; nrho = 2000      # 不知道什么意思,但是源代码中默认设置成了2000,尊重原作者

    # atnames = args.name
    # nr = args.nr
    # nrho = args.nrho

    for n in atnames:       # atnames为从命令行接收到的元素的名称,
        try:
            Database[n]     # 从另一个py文件导入Database这个字典, 其中键值是元素名称, 映射值是AtType类
        except KeyError:
            output = "Element {} not found in database.\n".format(n)
            valid = "Supported elements are: {}".format(" ".join(Database.keys()))
            sys.exit("".join([output, valid]))
            
    # 以上均为从命令行窗口接受参数和提供帮助等命令,以下才是真正的计算并输出的过程
